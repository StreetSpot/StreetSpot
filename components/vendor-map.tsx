...
function createVendorIcon(vendor: Vendor) {
  ...
  const countdownAnim =
    closing && !isPremium ? "animation: countdown-urgent 1.2s ease-in-out infinite;" : ""
      ...
      <div className="flex items-start gap-3">
        <div
          className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
            vendor.isPremium ? "bg-amber-500/15 ring-1 ring-amber-500/30" : "bg-primary/10"
          }`}
        >
          {vendor.isPremium ? (
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-3 text-xs text-muted-foreground">
            <span className={`flex items-center gap-1 font-mono ${closing ? "text-orange-400" : "text-muted-foreground"}`}>
      ...
      .forEach((spot) => {
        ...
        const status = spot.claimed
          ? spot.claimedBy
            ...
            : "Open gem"
          : "Unclaimed — owner can claim in app"
        const marker = L.marker([spot.lat, spot.lng], {
          icon: createGemIcon(spot),
          zIndexOffset: 100,
        })
          .addTo(mapRef.current!)
          .bindPopup(
    ...
    <div className="relative flex h-[calc(100dvh-57px)] flex-col">
      <div className="flex items-center justify-between border-b border-border bg-card px-4 py-2.5">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-md border border-border bg-secondary px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted"
        >
          {showList ? (
            <>
      ...
      <div className="relative flex-1">
            ...
            {standardVendors.length === 0 && premiumVendors.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Check back soon · pin gems in Discover & Tools
                </p>
              </div>
            ) : (
              <div className="flex flex-col gap-2">
        ...
        {selectedVendor && !showList && (
          <div
            className={`absolute bottom-4 left-4 right-4 z-[500] max-h-[70vh] overflow-y-auto rounded-xl border bg-card p-4 shadow-xl shadow-black/40 md:left-auto md:right-4 md:max-w-sm ${
              selectedVendor.isPremium ? "border-amber-500/30" : "border-border"
            }`}
          >
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {selectedVendor.isPremium && (
                      <span className="rounded-full bg-amber-500/10 px-1.5 py-0.5 text-[9px] font-bold uppercase text-amber-500 ring-1 ring-amber-500/20">
                        Gold
                      </span>
                    )}
                  </div>
        ...
        {selectedGem && !showList && !selectedVendor && (
          <div className="absolute bottom-4 left-4 right-4 z-[500] rounded-xl border border-border bg-card p-4 shadow-xl shadow-black/40 md:left-auto md:right-4 md:max-w-sm">
            <div className="mb-2 flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-primary">
                  {SPOT_TYPE_LABELS[selectedGem.type]}
                  {selectedGem.claimed
                    ? selectedGem.claimedBy
                      ? ` · Claimed by ${selectedGem.claimedBy}`
                      : ""
                    : " · Unclaimed"}
                </p>
              </div>
            ...
            {selectedGem.description && (
              <p className="mb-2 text-xs text-muted-foreground">{selectedGem.description}</p>
            )}
            <p className="mb-2 text-[11px] text-muted-foreground">
              Spotted by {selectedGem.createdBy}
            </p>
            <a
              href={`https://www.google.com/maps/dir/?api=1&destination=${selectedGem.lat},${selectedGem.lng}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex h-9 w-full items-center justify-center gap-2 rounded-lg bg-primary text-xs font-medium text-primary-foreground"
            >
              <Navigation className="h-3.5 w-3.5" />
              Get Directions
            </a>
